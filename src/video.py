...
print(f"SCRIPT CHARACTERS: {len(script_text)}")
...
required_clips = min(
    int(audio_duration / SCENE_SECONDS) + 1,
    MAX_CLIPS,
)

print(f"TARGET CLIPS: {required_clips}")

# ============================================================
# SEARCH PEXELS — WITH GLOBAL NO-REPEAT HISTORY
# ============================================================
...
print(f"PREVIOUSLY USED PEXELS VIDEOS: {len(used_video_ids)}")
...
for query in search_queries:
    ...
    try:
        ...
        for result in results:
            ...
            # NEVER reuse a video from an earlier topic/run.
            if str(video_id) in used_video_ids:
                print(f"SKIP REPEAT PEXELS ID: {video_id}")
                continue

            # Also avoid duplicate candidates in this run.
            if str(video_id) in run_video_ids:
                continue

            videos.append(result)
            run_video_ids.add(str(video_id))
...
if len(videos) < 3:
    print("NOT ENOUGH NEW HIGH QUALITY VIDEOS FOUND")
    exit(1)

print(f"TOTAL NEW CANDIDATE SOURCES: {len(videos)}")
...
for index, video in enumerate(videos, start=1):
    ...
    if not video_url:
        print(f"Candidate {index}: No 1080p/4K source")
        continue
    ...
    try:
        ...
        if not is_colourful_enough(output_file):
            print(f"Removing dull clip: {output_file.name}")
        ...
        print(
            f"ACCEPTED CLIPS: "
            f"{len(downloaded)}/{required_clips}"
        )
...
if len(downloaded) < 3:
    print("FAILED: LESS THAN 3 COLOURFUL HIGH QUALITY CLIPS")
    exit(1)

print(f"FINAL ACCEPTED CLIPS: {len(downloaded)}")

# Save ONLY successfully accepted clips.
save_used_video_ids(accepted_video_ids)

# ============================================================
# SCENE LENGTH
# ============================================================

scene_duration = audio_duration / len(downloaded)
...
for index, source in enumerate(downloaded, start=1):
    prepared = clips_dir / f"{topic_id}_clip_{index}.mp4"

    print("=" * 50)
    print(f"PROCESSING CLIP {index}/{len(downloaded)}")
    print(f"SOURCE: {source.name}")

    create_clip(source, prepared, scene_duration)
...
print("=" * 70)
print("JOINING COLOURFUL HIGH QUALITY CLIPS...")

combine_clips(prepared_clips, silent_video)

# ============================================================
# ADD NARRATION + LOW BACKGROUND MUSIC
# ============================================================

final_video = videos_dir / f"{topic_id}.mp4"

print("=" * 70)
print("ADDING TELUGU NARRATION + LOW BACKGROUND MUSIC...")
...
print(f"MOVING CLIPS: {len(prepared_clips)}")
print("COLOURFUL FOOTAGE: PRIORITIZED")
print("DULL/DARK FOOTAGE: FILTERED")
print("REPEATED PEXELS FOOTAGE: BLOCKED")
print("TELUGU NARRATION: YES")
print("BACKGROUND MUSIC: VERY LOW")
print(f"MUSIC VOLUME: {MUSIC_VOLUME}")
print("HIGH QUALITY SOURCE: 1080P/4K")
print("=" * 70)
